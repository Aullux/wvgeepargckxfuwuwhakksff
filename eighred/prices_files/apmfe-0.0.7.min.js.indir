...
Sentry._init=Sentry.init,Sentry.init=function(a){var b,c,d,e,f,g,h,i;if(!a.dsn)return Sentry._init(a),void 0;if(b=a.dsn.replace(/(https:)|(http:)/,document.location.protocol).replace(/@.*com?/,"@"+document.domain+"/apmfe"),a.dsn=b,a.sampleRate=.3,c=window._okGlobal||window.okGlobal,d=c?c.envSign:"",d&&(a.environment=d),e=RegExp(/daily/),!d.match(e)){Sentry._init(a);try{f=localStorage.getItem("ok_global"),g=f?JSON.parse(f):{},h=g.profile||{},i=h.uuid,i&&Sentry.setUser({id:i})}catch(j){}}};
